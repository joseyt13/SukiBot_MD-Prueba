...
let handler = async (m, { conn, usedPrefix, command, text}) => {
  ...
  // Detectar si el texto viene de una imagen citada
  let query = text?.trim();
  if (!query && m.quoted?.mimetype?.startsWith('image/') && m.quoted?.text) {
    query = m.quoted.text.replace(/\.apk/i, '').trim();
}

  if (!query) {
    return conn.reply(
      m.chat,
      `🌸 *𝖧𝗈𝗅𝖺 𝗉𝗋𝖾𝖼𝗂𝗈𝗌𝗎𝗋𝖺 ${name}~* 𝖭𝖾𝖼𝖾𝗌𝗂𝗍𝗈 𝗊𝗎𝖾 𝗆𝖾 𝖽𝗂𝗀𝖺𝗌 𝗅𝖺 𝖺𝗉𝗉 𝗊𝗎𝖾 𝗊𝗎𝗂𝖾𝗋𝖾𝗌 𝖻𝗎𝗌𝖼𝖺𝗋.\n\n💖 𝖤𝗃𝖾𝗆𝗉𝗅𝗈: ${usedPrefix + command} whatsapp\n🖼️ También puedes responder a una imagen con: *.apk nombreApp*`,
      m,
  ...
  try {
    ...
    if (!results?.length) {
      return conn.reply(
        m.chat,
        `💔 *𝖫𝗈 𝗌𝗂𝖾𝗇𝗍𝗈 ${name}~* 𝖭𝗈 𝖾𝗇𝖼𝗈𝗇𝗍𝗋𝗈́ 𝗋𝖾𝗌𝗎𝗅𝗍𝖺𝖽𝗈𝗌 𝗉𝖺𝗋𝖺 "${query}".`,
        m,
        { contextInfo, quoted: m}
);
}
    ...
    if (!data?.dllink) {
      return conn.reply(
        m.chat,
        `😭 *𝖲𝗎𝗄𝗂 𝗇𝗈 𝗉𝗎𝖾𝗱𝗈 𝗈𝖻𝗍𝖾𝗇𝖾𝗋 𝖾𝗅 𝖾𝗇𝗅𝖺𝖼𝖾 𝗉𝖺𝗋𝖺 "${results[0].name}".*`,
        m,
        { contextInfo, quoted: m}
);
}
